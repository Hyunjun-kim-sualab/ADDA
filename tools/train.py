...
@click.option('--lr', default=1.0e-4)
...
def main(dataset, split, model, output, gpu, iterations, batch_size, display,
    ...
    split_name = split
    dataset_split = getattr(adda.data.get_dataset(dataset_name), split_name)
    # dataset_2 = getattr(adda.data.get_dataset("svhn_sua"), split_name)
    model_fn = adda.models.get_model_fn(model)
    im, label = dataset_split.tf_ops()
    print(tf.shape(im))
